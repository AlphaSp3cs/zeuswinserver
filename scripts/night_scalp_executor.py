...
def is_crypto(symbol):
    ...
    if not info:
        return False
    return "crypto" in (info.path or "").lower()
...
def main():
    ...
    for c in candidates:
        ...
        if sl is None or tp is None or sl <= 0 or tp <= 0:
            if side == "SELL":
                sl = round(tick.bid - 2 * atr, info.digits)
                tp = round(tick.bid + 4 * atr, info.digits)
            else:
                sl = round(tick.ask + 2 * atr, info.digits)
                tp = round(tick.ask - 4 * atr, info.digits)
        ...
        if side == "BUY":
            limit_price = round(price - limit_offset, info.digits) if price - limit_offset > 0 else round(price, info.digits)
            otype = mt5.ORDER_TYPE_BUY_LIMIT
        else:
            limit_price = round(price + limit_offset, info.digits)
