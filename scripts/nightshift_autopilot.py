#!/usr/bin/env python3
"""
Nightshift Autopilot — scan/set/close scalps/intraday trades until hard cutoff.
Windows Task Scheduler should call this every N minutes during nightshift.
"""
import json, os, sys, time, glob, math, datetime
...
STATE_FILE = BASE_FIRM / 'workflow' / 'nightshift_state.json'
LOG_FILE = BASE_FIRM / 'workflow' / 'nightshift_log.jsonl'
OUT_DIR = BASE_FIRM / 'workflow'

CYCLE_SEC = 900          # 15 min between cycles
MAX_OPEN = 4             # max concurrent open positions
MAX_HOLD_SEC = 7200      # 2 hours max hold per scalp
RISK_PCT = 0.01          # 1% per trade
DEVIATION = 20
MAGIC = 20260812

# Nightshift window in ET: 16:00 -> 08:30 next day Sun-Thu, Fri 00:00-08:29
# Script enforces hard cutoff regardless of who/what called it.
CUTOFF_ET_HOUR = 8
CUTOFF_ET_MIN = 30
START_ET_HOUR = 16

# MT5 profiles from isolated connector
sys.path.insert(0, str(BASE_FIRM / 'scripts'))
from mt5_connect import connect as mt5_connect, KNOWN_BROKERS

BROKER_WATERFALL = ['etoro', 'ibkr', 'capital', 'ftmo']  # eToro demo first, then IBKR paper, then MT5

# Nightshift asset class: always-open
NIGHTSHIFT_UNIVERSE = {
...
def toast(title, msg, urgency='INFO'):
    if not HAS_WINOTIFY:
        return
    try:
        n = Notification(app_id="Zeus Nightshift", title=title, msg=msg)
        if urgency == 'HIGH':
...
def news_sentiment(symbol, asset_class):
    """Lightweight news sentiment via web search."""
    try:
        query = f"{symbol} {asset_class} market news today"
        # Use requests to avoid extra tool calls; simple search via DuckDuckGo lite
...
def size_by_risk(entry, sl, equity):
    if abs(entry - sl) <= 0:
        return None
    # Use crude tick-value approximation: 1 unit * 1 pip = ~$0.0001 per unit for most FX/CFDs
    # For crypto CFDs use actual MT5 tick_value if available
    risk_usd = equity * RISK_PCT
    # We’ll set volume after we know symbol info; here return ideal risk usd
    return risk_usd
...
def close_position(profile, position):
    if profile not in ('capital', 'ftmo'):
        return False, f'skip_close_non_mt5:{profile}'
    m = mt5_connect(profile)
    if not m:
...
# ── eToro / IBKR Connectors ──────────────────────────────────────────────────

def normalize_etoro_symbol(raw):
    """Convert yfinance-style symbols to eToro instrument symbols."""
    sym = str(raw).upper()
    # Crypto: LINK-USD -> LINK, ETH-USD -> ETH, BTC-USD -> BTC
    if '-' in sym and sym.endswith('USD'):
        sym = sym.replace('-USD', '')
    elif sym.endswith('USD') and len(sym) > 6:
        sym = sym[:-3]
    # Index: ^GSPC -> GSPC, ^RUT -> RUT, ^VIX -> VIX
    if sym.startswith('^'):
        sym = sym[1:]
    # Futures: ES=F -> ES, RTY=F -> RTY, YM=F -> YM, GC=F -> GC, HG=F -> HG, PL=F -> PL
    if sym.endswith('=F'):
        sym = sym[:-2]
    # Forex: EURUSD=X -> EURUSD, USDCHF=X -> USDCHF, etc.
    if sym.endswith('=X'):
        sym = sym[:-2]
    # Commodities with =X suffix
    if sym.endswith('=X'):
        sym = sym[:-2]
    return sym
...
def broker_executable(profile):
    if profile == 'etoro':
        return etoro_demo_executable()
    if profile == 'ibkr':
        return ibkr_paper_executable()
    return broker_executable_mt5(profile)

def broker_executable_mt5(profile):
    try:
        m = mt5_connect(profile)
        if not m:
            return False, 'no_connection'
...
def send_ticket_ibkr(symbol, side, qty, entry, sl, tp):
    ...
    try:
        ...
        elif sym in ('USDNOK', 'GBPNZD', 'USDSGD', 'USDCAD', 'USDSEK', 'USDMXN', 'USDZAR', 'USDPLN', 'USDTRY'):
            base = sym[:3]
            quote = sym[3:]
...
def run_cycle():
    ...
    # 1. Hard cutoff / outside nightshift window guard
    # Nightshift window: 16:00 ET -> 08:30 ET next day (overnight, crosses midnight)
    in_window = et.hour >= START_ET_HOUR or et.hour < CUTOFF_ET_HOUR or (et.hour == CUTOFF_ET_HOUR and et.minute < CUTOFF_ET_MIN)
    if not in_window:
        msg = f"Outside nightshift window. Now {et.strftime('%H:%M ET')}. No action."
        print(msg)
        log_json({'event': 'OUTSIDE_WINDOW', 'msg': msg})
        toast('Nightshift IDLE', msg, 'INFO')
        save_state({'cycles': cycle, 'open': [], 'last_run': now.isoformat(), 'stopped': True})
        return

    # 2. Broker executable probe (cached briefly)
    broker_ok = {}
    for profile in BROKER_WATERFALL:
    ...
    if not executable_brokers:
        ...
        toast('Nightshift BLOCKED', msg, 'HIGH')
        save_state(state)
        return

    # 3. Close expired scalps
    open_positions = state.get('open', [])
    open_positions, expired = close_all_expired(open_positions)
    if expired:
        toast('Nightshift CLOSE', f"Closed {len(expired)} expired scalp(s).", 'HIGH')

    # 4. Load setups from scan artifacts: scandata/setups.json + workflow/*scan*.json
    setups = []
    scan_sources = []
    ...
    # Filter to executable symbols per broker support
    executable_symbols = set()
    for profile in executable_brokers:
        if profile == 'etoro':
            executable_symbols.update(ETORO_INSTRUMENT_MAP.keys())
        elif profile in ('ibkr', 'capital', 'ftmo'):
            # MT5/IBKR can handle broader set; we'll validate per symbol in send_ticket
            pass

    # If no scan artifacts found, fall back to lightweight MT5/IBKR/eToro scan
    if not setups:
        log_json({'event': 'NO_SCAN_ARTIFACTS', 'msg': 'Falling back to live symbol scan'})
        toast('Nightshift SCAN', 'No scan artifacts found, using live scan', 'INFO')
        for cls, syms in NIGHTSHIFT_UNIVERSE.items():
            for sym in syms:
    ...
    toast('Nightshift SCAN', f"Loaded {len(setups)} setups from {len(scan_sources)} sources", 'INFO')
    ...
    for setup in sorted(setups, key=lambda x: x.get('conviction', 0), reverse=True)[:slots]:
        ...
        for profile in executable_brokers:
            ...
            if res:
                ...
                break  # stop broker loop after first fill
            else:
    ...
    toast('Nightshift CYCLE', summary, 'HIGH' if placed > 0 else 'INFO')

    # Save cycle artifact
    ts = now.strftime('%Y%m%d%H%M')
    artifact = OUT_DIR / f'nightshift_cycle_{ts}.json'
...
def main():
    print(f"Nightshift autopilot started at {now_utc().isoformat()}")
    et = et_now()
    # Nightshift window: 16:00 ET -> 08:30 ET next day (overnight, crosses midnight)
    in_window = et.hour >= START_ET_HOUR or et.hour < CUTOFF_ET_HOUR or (et.hour == CUTOFF_ET_HOUR and et.minute < CUTOFF_ET_MIN)
    if not in_window:
